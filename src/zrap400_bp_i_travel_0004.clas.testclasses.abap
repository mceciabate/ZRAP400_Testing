...
*  ENDMETHOD.
  ...
  METHOD set_status_to_accepted.
    ...
    DATA: result      TYPE TABLE    FOR ACTION RESULT ZRAP400_I_Travel_0004~acceptTravel,
  ...
  METHOD set_status_to_rejected.
    ...
    DATA: result      TYPE TABLE    FOR ACTION RESULT ZRAP400_I_Travel_0004~rejectTravel,
          mapped      TYPE RESPONSE FOR MAPPED EARLY ZRAP400_I_Travel_0004,
          failed      TYPE RESPONSE FOR FAILED EARLY ZRAP400_I_Travel_0004,
          reported    TYPE RESPONSE FOR REPORTED EARLY ZRAP400_I_Travel_0004,
          entity_keys TYPE STANDARD TABLE OF ty_entity_key.
    ...
    " expect input keys and output keys to be same and OverallStatus everywhere = 'A' (Accepted)
    DATA exp LIKE result.
    exp = VALUE #(  ( TravelID = 42  %param-TravelID = 42  %param-OverallStatus = 'X' )
                    ( TravelID = 43  %param-TravelID = 43  %param-OverallStatus = 'X' )
                    ( TravelID = 44  %param-TravelID = 44  %param-OverallStatus = 'X' ) ).
